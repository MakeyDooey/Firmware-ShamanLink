...
#if ((defined FSL_FEATURE_SOC_USBPHY_COUNT) &&                                 \
     (FSL_FEATURE_SOC_USBPHY_COUNT > 0U))
#include "usb_phy.h"
#endif
#if defined(FSL_FEATURE_USB_KHCI_KEEP_ALIVE_ENABLED) &&                        \
    (FSL_FEATURE_USB_KHCI_KEEP_ALIVE_ENABLED > 0U) &&                          \
    defined(USB_DEVICE_CONFIG_KEEP_ALIVE_MODE) &&                              \
    (USB_DEVICE_CONFIG_KEEP_ALIVE_MODE > 0U) &&                                \
...
#if (defined(USB_DEVICE_CONFIG_LPCIP3511HS) &&                                 \
 ...
 *
 * This function handles the CDC class specific requests. For the current case,
 * device is waiting for data from host. Once device receives data,
 * kUSB_DeviceCdcEventRecvResponse event will be asserted. In
 * kUSB_DeviceCdcEventRecvResponse event, the received data lenght is saved to
 * s_recvSize. If s_recvSize is 0, device will call USB_DeviceCdcAcmRecv to
 * schedule buffer for next receiving event directly and it means there is no
 * data to echo to host. If s_recvSize is not 0, USB_DeviceCdcVcomTask will copy
 * the received data into s_currSendBuf then send back to host. Once data is
 * echoed back completely, USB_DeviceCdcAcmRecv is called in
 * kUSB_DeviceCdcEventSendResponse event to be ready to receive the next data
 * from host. Instead, USB_DeviceCdcAcmRecv also can be called in
 * kUSB_DeviceCdcEventRecvResponse event as long as the received data is handled
 * completely.
 *
...
usb_status_t USB_DeviceCdcVcomCallback(class_handle_t handle, uint32_t event,
...
#if ((defined USB_DEVICE_CONFIG_CDC_CIC_EP_DISABLE) &&                         \
     (USB_DEVICE_CONFIG_CDC_CIC_EP_DISABLE > 0U))
#else
  uint32_t len;
#endif
  uint8_t *uartBitmap;
    ...
    if ((epCbParam->length != 0) &&
      ...
      error =
          USB_DeviceCdcAcmSend(handle, USB_CDC_VCOM_BULK_IN_ENDPOINT, NULL, 0);
               ...
               (1U == s_cdcVcom.startTransactions)) {
  ...
  case kUSB_DeviceCdcEventRecvResponse: {
    ...
    (FSL_FEATURE_USB_KHCI_USB_RAM > 0U)
      ...
      if (epCbParam->length > 0) {
        xStreamBufferSend(xRxStreamBuffer, epCbParam->buffer, epCbParam->length,
                          0);
      }

      error = USB_DeviceCdcAcmRecv(
          handle, USB_CDC_VCOM_BULK_OUT_ENDPOINT, s_currRecvBuf,
  ...
  case kUSB_DeviceCdcEventSetControlLineState: {
    ...
    acmInfo->serialStateBuf[1] =
        USB_DEVICE_CDC_NOTIF_SERIAL_STATE; /* bNotification */
    ...
    if (0U == ((usb_device_cdc_acm_struct_t *)handle)->hasSentState) {
      error = USB_DeviceCdcAcmSend(handle, USB_CDC_VCOM_INTERRUPT_IN_ENDPOINT,
...
usb_status_t USB_DeviceCallback(usb_device_handle handle, uint32_t event,
  ...
  case kUSB_DeviceEventBusReset: {
    ...
    (defined(USB_DEVICE_CONFIG_LPCIP3511HS) &&                                 \
     (USB_DEVICE_CONFIG_LPCIP3511HS > 0U))
...
static void CDC_VCOM_FreeRTOSExitCritical(uint32_t sr) { EnableGlobalIRQ(sr); }
 ...
 * @return None.
 */
/* The CLI commands are now dynamically mapped through cli_commands.c */

/*!
 * @brief USB TX task.
 */
void USB_Tx_Task(void *handle) {
  /* s_currSendBuf is used here to hold data while transmitting via DMA/USB PHY
   */
  ...
  while (1) {
    if ((1U == s_cdcVcom.attach) && (1U == s_cdcVcom.startTransactions)) {
      /* Wait for TX mechanism to be idle before pulling next chunk */
      if (xSemaphoreTake(xTxSemaphore, portMAX_DELAY) == pdTRUE) {
        count = xStreamBufferReceive(xTxStreamBuffer, s_currSendBuf,
                                     sizeof(s_currSendBuf), portMAX_DELAY);
        if (count > 0) {
          USB_DeviceCdcAcmSend(s_cdcVcom.cdcAcmHandle,
                               USB_CDC_VCOM_BULK_IN_ENDPOINT, s_currSendBuf,
                               count);
        } else {
          xSemaphoreGive(xTxSemaphore);
        }
...
/*!
 * @brief Application CLI task function.
 */
  ...
  while (1) {
    if (xStreamBufferReceive(xRxStreamBuffer, &cRxedChar, 1, portMAX_DELAY) >
      ...
      if (cRxedChar == '\r' || cRxedChar == '\n') {
        ...
        if (ucInputIndex > 0) {
          ...
          do {
            ...
            if (strlen(pcOutputString) > 0) {
              xStreamBufferSend(xTxStreamBuffer, pcOutputString,
                                strlen(pcOutputString), portMAX_DELAY);
            }
        ...
        if (ucInputIndex > 0) {
          ucInputIndex--;
          xStreamBufferSend(xTxStreamBuffer, "\b \b", 3, portMAX_DELAY);
        }
      } else {
        if (ucInputIndex < sizeof(cInputString) - 1 && cRxedChar >= ' ' &&
            cRxedChar <= '~') {
          cInputString[ucInputIndex++] = cRxedChar;
          xStreamBufferSend(xTxStreamBuffer, &cRxedChar, 1, portMAX_DELAY);
        }
      }
    }
  }
...
{
  BOARD_InitHardware();
  /* Ensure initial LED state is HIGH (Off) using BOARD_InitBootPins -> done
   * securely in generated code */
  ...
  if (s_cdcVcom.deviceHandle) {
    if (xTaskCreate(USB_DeviceTask, "usb device task",
                    2048L / sizeof(portSTACK_TYPE), s_cdcVcom.deviceHandle,
                    APP_USB_TASK_PRIORITY,
                    &s_cdcVcom.deviceTaskHandle) != pdPASS) {
  ...
  if (xTaskCreate(CLI_Task, "cli task", 4096L / sizeof(portSTACK_TYPE), NULL,
                  APP_CLI_TASK_PRIORITY, NULL) != pdPASS) {
#if (defined(__CC_ARM) || (defined(__ARMCC_VERSION)) || defined(__GNUC__))
    return 1;
#else
    return;
  ...
  if (xTaskCreate(USB_Tx_Task, "usb tx task", 1024L / sizeof(portSTACK_TYPE),
                  NULL, APP_CLI_TASK_PRIORITY, NULL) != pdPASS) {
#if (defined(__CC_ARM) || (defined(__ARMCC_VERSION)) || defined(__GNUC__))
    return 1;
#else
    return;
